...
buses.df <- buses.df %>% rename(name = bus)
...
# filter out any buses that are not in the edges "from" or "to"
buses.df <- buses.df %>%
  filter(name %in% edges.df$from | name %in% edges.df$to)
# from 296015 to 296015 > none removed!!
```
...
graph
saveRDS(graph, "10x/variablesdss/graph.rds")
```

## Check the Number of Components in the Network
```{r}
# read in the network
graph <- readRDS("10x/variablesdss/graph.rds")
...
# 1 component with 80,644 nodes, then everything else is less than 150 nodes per component
