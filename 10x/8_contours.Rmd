...
buses.sf <- readRDS("10x/variables/buses_sf.rds")
lines.buffer <- readRDS("10x/variables/lines_buffer_1000m.rds")

# Convert the lines buffer to a vector
lines.buffer.vec <- vect(lines.buffer)
saveRDS(lines.buffer.vec, "10x/variables/lines_buffer_1000m_vec.rds")
...
rm(buses.sf, buses.df, lines.buffer, ext, grd, neighbors, mg)
```

## Prepare Contour for Plotting
```{r}  
# read in data
contour <- readRDS("10x/variables/contour_n10_rast.rds")
lines.buffer.vec <- readRDS("10x/variables/lines_buffer_1000m_vec.rds")
...
rm(contour.mask, contour.df, contour, lines.buffer.vec)
```
Rows: 3,553,912
Columns: 3
...
ggsave(
  filename = "10x/figures/contour_plot_mask_n10.png", 
  contour_plot,
  width = 6480,
  height = 6480,
  dpi = 300,
