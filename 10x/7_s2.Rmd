...
for (res in min_res:max_res) {
  s2_col <- paste0("s2r", res)
  buses.s2[[res]] <- buses.sf %>% group_by(!!sym(s2_col)) %>% summarise(
...
glimpse(buses.s2)
# export
saveRDS(buses.s2, "10x/variables/buses_s2_r8to12.rds")
```

## Visualize Hexes
```{r}
# import buses.s2 and lines
buses.s2 <- readRDS("10x/variables/buses_s2_r8to12.rds")
# plot just desired resolutions
buses.s2 <- buses.s2 %>% filter(res == 9)
glimpse(buses.s2)
...
ggsave(
  filename = "10x/figures/s2_plot_r9.png", 
  s2plot,
  width = 6840,
  height = 6480,
  dpi = 300,
