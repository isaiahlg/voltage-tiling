...
# saveRDS(buses.df, paste(csv_path, "buses_df.rds", sep = "/"))
# buses.df$region <- as.factor(buses.df$region)
# buses.p13u.df <- buses.df %>% filter(region == "P13U")
# saveRDS(buses.p13u.df, paste(csv_path, "P13U", "buses_df.rds", sep = "/"))
...
lines.sf <- lines.sf %>% 
    rename(geometry = wkt)
glimpse(lines.sf)
# drop columns region.x and region.y
...
# read in the P13U buses and lines
buses.p13u.df <- read.csv(paste(csv_path, "P13U", "buses_df.csv", sep = "/"), header = TRUE, sep = ",")
tx.p13u.df <- read.csv(paste(csv_path, "P13U", "tx_df.csv", sep = "/"), header = TRUE, sep = ",")
glimpse(tx.p13u.df)
# find any buses that contain ".1.2.3" in their name
# buses.p13u.df %>% filter(grepl("\\.", bus)) %>% select(bus)
...
tx.p13u.df <- tx.p13u.df %>% select(-region.x, -region.y)
...
# read in the P13U buses and lines
buses.df <- read.csv(paste(csv_path, "buses_df.csv", sep = "/"), header = TRUE, sep = ",")
tx.df <- read.csv(paste(csv_path, "tx_df.csv", sep = "/"), header = TRUE, sep = ",")
glimpse(tx.df)
